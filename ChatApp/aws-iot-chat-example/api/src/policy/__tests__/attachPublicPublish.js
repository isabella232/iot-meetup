...
describe('attach public publish policy', () => {
  ...
  const identityId = 'eu-west-1:identity-id';
  const event = {
    requestContext: {
      identity: {
  ...
  const context = {
    invokedFunctionArn: 'arn:aws:lambda:eu-west-1:123456789:function:chat-app-api-prod-AttachPublicPublishPolicy',
  };

  describe('when iot calls succeed', () => {
    beforeEach(() => {
      const expectedPolicyName = 'PublicPublishPolicy.eu-west-1-identity-id';
